...
function calculateCostTotalNight(costPerNight, startDate, endDate) {
    return getTotalNights(startDate, endDate) * costPerNight;
}

function calculateBreakFastCost(breakfast, cardNumber, peopleNumber, startDate, endDate) {
    if (breakfast == "yes" && !notEmpty(cardNumber)) {
        return peopleNumber * BREAKFAST_PRICE * getTotalNights(startDate, endDate);
    } else {
        return 0;
    }
}
...
function getInfo() {
    ...
    let breakfastCostValue = calculateBreakFastCost(breakfastValue, cardNumberValue, peopleNumberValue, startDateValue, endDateValue);
